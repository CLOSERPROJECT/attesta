...
- **Component tiers:** full template components (`templates/components/` + view struct in `components.go`); **CSS-only components** (see `docs/css.md`) — reused markup with a dedicated CSS module and inline HTML, no full template partial (examples: page-header in `page-header.css` with optional `page_header_back` micro-partial, panel in `panel.css`, dialog in `dialog.css`, list-row in `list-row.css`); primitives in `shared.css`. Migrate one at a time.
