...
**Decision:** If a platform-admin save’s semantic diff is only presentation fields, skip `DeleteWorkflowData` and keep existing instances. Any other change (or a compare that cannot prove presentation-only) still purges. Live UI shows the current blueprint labels on old instances.
