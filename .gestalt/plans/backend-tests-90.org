...
* DONE [#A] Raise coverage on pure helpers + deterministic logic
  - Effort :: M
...
** DONE [#B] Cover attachment helpers (`sanitizeAttachmentFilename`, `detectAttachmentContentType`)
...
* DONE [#A] Expand handler coverage to remaining branches (still unit tests)
  - Effort :: M
...
** DONE [#A] Cover JSON endpoints (`handleNotarizedJSON`, `handleMerkleJSON`)
...
** DONE [#A] Cover file download paths (`handleDownloadSubstepFile`, `handleDownloadAllFiles`)
...
** DONE [#B] Cover action error rendering (`renderActionError` + request variant)
...
* DONE [#A] Unit-test the Cerbos authorizer adapter (no Cerbos required)
  - Effort :: S
...
** DONE [#A] Add `CerbosAuthorizer.CanComplete` tests (request + response mapping)
...
* DONE [#A] Unit-test the Mongo store adapter (no MongoDB required)
  - Effort :: L
...
** DONE [#A] Introduce a minimal “mongo driver port” seam (Hex-style, internal)
...
** DONE [#A] Add MongoStore unit tests for process + notarization CRUD
...
** DONE [#A] Add MongoStore unit tests for attachments (GridFS)
...
* DONE [#A] Update the coverage gate and keep it ergonomic
  - Effort :: S
...
** DONE [#A] Raise Taskfile gate from 70% to 90%
...
** DONE [#B] Document the new expectation (90%) where developers look
...
* DONE [#C] Optional: keep a thin integration slice (not part of the gate)
  - Effort :: S
...
** DONE [#C] Add/maintain integration tests behind `integration` build tag
