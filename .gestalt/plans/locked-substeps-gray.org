...
* WIP [#A] Add muted styling for locked action cards
  :PROPERTIES:
  :Effort: 1.0h
  ...
  :END:
** WIP [#A] Style `.action-card.action-locked` and disabled Formata host
   :PROPERTIES:
   ...
   :END:
** TODO [#B] (Optional) Make the “Open form builder” link clearly inactive when locked
   :PROPERTIES:
...
* TODO [#B] Add regression tests for “locked Formata is disabled”
  :PROPERTIES:
  :Effort: 1.0h
  ...
  :END:
** TODO [#B] Unit test `buildActionList` for locked + Formata disabled
   :PROPERTIES:
   ...
   :END:
** TODO [#B] Template render test for `data-formata-disabled="true"` on locked actions
   :PROPERTIES:
...
* TODO [#C] Ship the updated frontend bundle
  :PROPERTIES:
  :Effort: 0.5h
  ...
  :END:
** TODO [#C] Rebuild Vite output and verify locally
   :PROPERTIES:
