...
* DONE [#B] Make workflow picker landing full-width and card grid responsive
Effort: S
Goal: Improve `/` and `/backoffice` workflow picker layout so the landing panel uses the available width, cards have a consistent “designed” size, and mobile renders one card per row.
Notes: Apply changes only to workflow picker pages; keep other `.landing` panels (e.g. backoffice department picker) at their current width unless explicitly requested.
** DONE [#A] Add a dedicated “wide picker” class to the picker panels
Why: `.landing` is capped (`max-width`) and is used by multiple pages; the workflow picker should be full-width without affecting other landings.
Change: In `server/templates/home.html` (`home_picker_body`) and `server/templates/backoffice.html` (`backoffice_picker_body`), add a class like `landing-wide` to the `<section class="panel landing ...">` wrapper. In `web/src/styles.css`, override only for that class (`.landing.landing-wide { max-width: none; width: 100%; }`) so the panel spans the available page width (still inside `.page` padding).
Tests: Update picker handler/template tests to assert the workflow picker section includes `landing-wide` while other landing pages still render without it.
Done when: Picker panels occupy full width inside `.page`, and other `.landing` panels remain unchanged.
** DONE [#A] Fix card sizing to target 4/3-per-row desktop, 1-per-row mobile
Why: The current `auto-fit` grid can produce awkward card widths on large screens, and can render 2-up on “large phones” unless explicitly constrained.
Change: In `web/src/styles.css`, change `.workflow-grid` to use explicit breakpoints and a constrained card width so cards keep a predictable size while the panel can be full width. Suggested targets: (1) phones: 1 column; (2) tablets: 2 columns; (3) desktop: 3 columns; (4) wide: 4 columns. Use `justify-content: center` and `grid-template-columns: repeat(N, minmax(240px, 280px))` (or similar) to avoid overly wide cards on large monitors.
Tests: No automated CSS layout tests; validate manually via responsive browser emulation at common widths (e.g. 390px phone, 768px tablet, 1024px desktop, 1440px wide).
Done when: Pickers show 1 card per row on phones, ~3 columns on typical desktop widths, ~4 on wide screens, and cards stay within the chosen size bounds without overflow.
** DONE [#B] Rebuild web bundle and do a quick UI smoke check
Why: The backend serves `web/dist` assets; CSS changes must be compiled into the Vite output for non-dev mode.
Change: Run `cd web && npm install && npm run build` (if needed) and verify the picker pages (`/` and `/backoffice`) in the browser for both light/dark themes.
Tests: Run `cd server && go test ./...` to ensure template/test assertions still pass after class/markup changes.
Done when: Built assets include the new layout, and backend tests remain green.
** DONE [#C] Left-align picker grid and use flexible `fr` columns
Why: Centered, width-capped tracks leave visible blank space at row start on wide panels.
Change: In `web/src/styles.css`, set `.workflow-grid` to `justify-content: start` and switch base/breakpoint column tracks from `minmax(240px, 280px)` to flexible `1fr`-based tracks.
Tests: Rebuild web bundle and run backend tests.
Done when: Cards start from the left edge and columns expand to fill available row width at each breakpoint.
