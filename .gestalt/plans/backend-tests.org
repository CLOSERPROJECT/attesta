...
* WIP [#A] Align on scope + success criteria
  - Effort :: S
...
** TODO [#B] Confirm “negative test” expectations
   ...
   - Tests :: N/A (decision).
   - Done when :: A matrix of required negative cases exists and is accepted.

* TODO [#A] Establish baseline + test harness conventions
  - Effort :: S
...
** TODO [#A] Inventory current testability seams
   ...
   - Tests :: N/A (inventory).
   - Done when :: The plan lists all IO seams and names the minimal interfaces/function seams to introduce.
** TODO [#B] Define test layout + naming
   ...
   - Done when :: A short “Testing conventions” section is included in the plan and later mirrored into docs.

* TODO [#A] Minimal refactor to enable fast unit tests (ports/adapters)
  - Effort :: M
...
** TODO [#A] Introduce a storage port with an in-memory adapter for tests
...
** TODO [#A] Introduce a Cerbos/authorization port + fake
...
** TODO [#B] Make time deterministic where it affects assertions
...
** TODO [#B] Provide template/config seams for handler tests
...
* TODO [#A] Build fixtures and test matrix (positive + negative)
  - Effort :: M
...
** TODO [#A] Define canonical workflow fixture + actors
...
** TODO [#A] Enumerate handler routes + expected behavior (REPR-style per route)
   ...
   - Tests :: The route matrix becomes the checklist for tests; each route must have at least 1 positive + 2 negative tests.
   - Done when :: The plan lists concrete test cases per route, including status codes and key assertions.

* TODO [#A] Implement unit tests for pure/domain-ish helpers
  - Effort :: M
...
** TODO [#A] Workflow ordering + progress key invariants
...
** TODO [#A] Sequence/availability state machine
...
** TODO [#B] Payload normalization + digest
...
** TODO [#B] Actor + HTMX detection helpers
...
* TODO [#A] Implement handler tests with fakes (positive + negative)
  - Effort :: L
...
** TODO [#A] `POST /process/start` tests
...
** TODO [#A] `GET /process/:id` + `GET /process/:id/timeline` tests
...
** TODO [#A] `POST /process/:id/substep/:substepId/complete` tests (HTMX + non-HTMX)
...
** TODO [#B] `GET /backoffice` + department dashboards tests
