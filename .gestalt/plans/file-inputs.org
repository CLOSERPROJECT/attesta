...
* WIP [#A] Confirm scope and UX decisions
  - Effort :: S
...
** TODO [#A] Capture confirmed download UX and auth policy
...
* TODO [#A] Extend workflow config semantics (`inputType: "file"`)
  - Effort :: S
...
** TODO [#A] Add a file substep to Step "1"
...
** TODO [#B] Validate/normalize `inputType` values on config load
...
* TODO [#A] Add file-upload handling to “complete substep”
  - Effort :: M
...
** TODO [#A] Define the request contract (HTML/HTMX)
...
** TODO [#A] Implement robust parsing + error mapping in `handleCompleteSubstep`
...
** TODO [#A] Ensure notarization covers file content integrity
...
* TODO [#A] Persist attachments and add a download endpoint
  - Effort :: M
...
** TODO [#A] Add an attachment model + store port
...
** TODO [#A] Add `GET /process/:id/substep/:substepId/file` (or equivalent) route
...
* TODO [#B] Update UI rendering for file substeps and timeline display
  - Effort :: M
...
** TODO [#A] Render the correct input control per `InputType`
...
** TODO [#B] Improve timeline display for file payloads
...
* TODO [#C] Documentation and operational notes
  - Effort :: S
  - Goal :: Make the new capability discoverable and safe to run locally.
  - Notes :: Keep docs minimal; this is a demo repo.
** TODO [#C] Document how to declare file inputs in YAML and how to use them
