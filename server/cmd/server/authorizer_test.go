...
func TestCerbosAuthorizerCanCompleteBuildsRequestAndMapsAllow(t *testing.T) {
  ...
  "resourceInstances": {
	...
	allowed, err := authorizer.CanComplete(context.Background(), Actor{UserID: "u1", Role: "dep1"}, "proc-1", WorkflowSub{
		SubstepID: "1.1",
		Order:     2,
		Role:      "dep1",
	}, 1, true)
...
func TestCerbosAuthorizerCanCompleteReturnsErrorForBadStatusAndBadJSON(t *testing.T) {
	...
	_, err := authorizer.CanComplete(context.Background(), Actor{UserID: "u1", Role: "dep1"}, "proc-1", WorkflowSub{
		SubstepID: "1.1",
		Order:     1,
		Role:      "dep1",
	}, 1, true)
