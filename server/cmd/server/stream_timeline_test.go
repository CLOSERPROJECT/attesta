...
func TestStreamTimelineTemplateRendersDoneSubstepMetaFromBody(t *testing.T) {
	tmpl := parseTestTemplates(t)

	view := testStreamTimelineView()
	view.Timeline[0].Substeps[0].Status = "locked"
	view.Timeline[0].Substeps[0].DoneAt = ""
...
func TestStreamTimelineTemplateRendersDoneSubstepMetaClasses(t *testing.T) {
	...
	if strings.Contains(body, `class="time"`) || strings.Contains(body, `class="actor"`) {
		t.Fatalf("expected substep-meta-* classes, not legacy time/actor classes, got: %s", body)
	}
}
