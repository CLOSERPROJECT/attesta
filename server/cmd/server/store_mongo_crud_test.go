...
func TestMongoStoreUpdateOrganizationProfile(t *testing.T) {
	currentOrgID := primitive.NewObjectID()
	oldSlug := "acme-org"
	newSlug := "renamed-org"

	orgCollection := &fakeMongoCollection{}
	orgCollection.findOneFn = func(ctx context.Context, filter interface{}, opts ...*options.FindOneOptions) mongoSingleResultPort {
		f, _ := filter.(bson.M)
		switch f["slug"] {
		case oldSlug:
			return fakeSingleResult{decodeFn: func(v interface{}) error {
				*(v.(*Organization)) = Organization{ID: currentOrgID, Name: "Acme Org", Slug: oldSlug}
				return nil
			}}
		case newSlug:
			// First lookup checks collisions (expect not found), second lookup reloads updated record.
			if len(orgCollection.updateOneUpdates) == 0 {
				return fakeSingleResult{err: mongo.ErrNoDocuments}
			}
			return fakeSingleResult{decodeFn: func(v interface{}) error {
				*(v.(*Organization)) = Organization{ID: currentOrgID, Name: "Renamed Org", Slug: newSlug, LogoAttachmentID: "logo-123"}
				return nil
			}}
		default:
			return fakeSingleResult{err: mongo.ErrNoDocuments}
	...
	rolesCollection := &fakeMongoCollection{
		findFn: func(ctx context.Context, filter interface{}, opts ...*options.FindOptions) (mongoCursorPort, error) {
			return &fakeAnyCursor{
				items: []interface{}{
					Role{ID: primitive.NewObjectID(), OrgSlug: oldSlug, Slug: "org-admin", Name: "Org Admin"},
					Role{ID: primitive.NewObjectID(), OrgSlug: oldSlug, Slug: "qa-reviewer", Name: "QA Reviewer"},
				},
			}, nil
		},
	}
	usersCollection := &fakeMongoCollection{
		findFn: func(ctx context.Context, filter interface{}, opts ...*options.FindOptions) (mongoCursorPort, error) {
			return &fakeAnyCursor{
				items: []interface{}{
	...
	db := &fakeMongoDatabase{
		collections: map[string]*fakeMongoCollection{
			...
			collectionRoles:         rolesCollection,
			collectionUsers:         usersCollection,
		},
	}
	store := &MongoStore{dbPort: db}

	updated, err := store.UpdateOrganizationProfile(t.Context(), oldSlug, "Renamed Org", "logo-123")
	if err != nil {
		t.Fatalf("UpdateOrganizationProfile returned error: %v", err)
	}
	if updated.Slug != newSlug {
		t.Fatalf("updated org slug = %q, want %q", updated.Slug, newSlug)
	}
	if len(orgCollection.updateOneUpdates) != 1 {
		t.Fatalf("expected one organization update call, got %d", len(orgCollection.updateOneUpdates))
	}
	if len(rolesCollection.updateOneUpdates) != 2 {
		t.Fatalf("expected two role update calls, got %d", len(rolesCollection.updateOneUpdates))
	}
	if len(usersCollection.updateOneUpdates) != 2 {
		t.Fatalf("expected two user update calls, got %d", len(usersCollection.updateOneUpdates))
	}
}

func TestMongoStoreUpdateOrganizationProfileValidationAndErrors(t *testing.T) {
	t.Run("empty name is rejected", func(t *testing.T) {
		store := &MongoStore{dbPort: &fakeMongoDatabase{}}
		if _, err := store.UpdateOrganizationProfile(t.Context(), "acme-org", "   ", ""); err == nil {
	...
	t.Run("duplicate slug is rejected", func(t *testing.T) {
		...
		collection := &fakeMongoCollection{
			findOneFn: func(ctx context.Context, filter interface{}, opts ...*options.FindOneOptions) mongoSingleResultPort {
				...
				case "other-name":
					return fakeSingleResult{decodeFn: func(v interface{}) error {
						*(v.(*Organization)) = Organization{ID: primitive.NewObjectID(), Name: "Other Name", Slug: "other-name"}
						return nil
					}}
				default:
					return fakeSingleResult{err: mongo.ErrNoDocuments}
		...
		store := &MongoStore{dbPort: &fakeMongoDatabase{collections: map[string]*fakeMongoCollection{
			collectionOrganizations: collection,
		}}}
		if _, err := store.UpdateOrganizationProfile(t.Context(), "acme-org", "Other Name", ""); err == nil || !strings.Contains(err.Error(), "already exists") {
			t.Fatalf("expected duplicate slug error, got %v", err)
		}
	})

	t.Run("missing org returns not found", func(t *testing.T) {
		collection := &fakeMongoCollection{
			findOneFn: func(ctx context.Context, filter interface{}, opts ...*options.FindOneOptions) mongoSingleResultPort {
				return fakeSingleResult{err: mongo.ErrNoDocuments}
			},
		...
		if _, err := store.UpdateOrganizationProfile(t.Context(), "missing-org", "Renamed", ""); !errors.Is(err, mongo.ErrNoDocuments) {
			t.Fatalf("expected mongo.ErrNoDocuments, got %v", err)
		}
	})
}

func TestMongoStoreSetUserOrganization(t *testing.T) {
	usersCollection := &fakeMongoCollection{}
	store := &MongoStore{dbPort: &fakeMongoDatabase{collections: map[string]*fakeMongoCollection{
		collectionUsers: usersCollection,
	}}}
	...
	if !reflect.DeepEqual(usersCollection.updateOneUpdates[0], wantUpdate) {
		t.Fatalf("update = %#v, want %#v", usersCollection.updateOneUpdates[0], wantUpdate)
	}
