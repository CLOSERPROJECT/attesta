...
func TestPlatformAdminTemplateOrganizationInviteAndPagination(t *testing.T) {
	...
	if !strings.Contains(body, `name="invite_email"`) || !strings.Contains(body, "Optional. Invite the first org admin now") {
	...
	prevIndex := strings.Index(body, ">Previous<")
	pageOneIndex := strings.Index(body, ">1<")
	pageThreeIndex := strings.Index(body, ">3<")
	nextIndex := strings.Index(body, ">Next<")
	if prevIndex != -1 {
		t.Fatalf("did not expect previous button on first page, got: %s", body)
	}
	if pageOneIndex == -1 || pageThreeIndex == -1 || nextIndex == -1 || nextIndex < pageThreeIndex {
